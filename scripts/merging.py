    ...
    def apply_merging_operation(
        ...
        for node1, node2 in self.merging_graph.edges:
            if not new_graph.has_edge(node1, node2):
                continue
            atom_indices = new_graph.nodes[node1]["atom_indices"].union(
                new_graph.nodes[node2]["atom_indices"]
            )
    ...
    def apply_merging_operation_producer(self, motif: str, q: Queue) -> None:
        ...
        for node1, node2 in self.merging_graph.edges:
            ...
            if motif_smiles == motif:
                graph_before_merge = new_graph.copy()
                merge_nodes(new_graph, node1, node2)
                update_stats_producer(
                    self, graph_before_merge, new_graph, node1, node2, q, self.idx
                )
...
def load_mols(train_path: str, num_workers: int) -> List[MolGraph]:
    ...
    print(
        f"[{datetime.now()}] Loading molecules finished. Total: {len(mols)} molecules.\n"
    )
    return mols
...
def merge_nodes(graph: nx.Graph, node1: int, node2: int) -> None:
    ...
    atom_indices = graph.nodes[node1]["atom_indices"].union(
        graph.nodes[node2]["atom_indices"]
    )
    for n in neighbors:
...
def get_stats_producer(batch: List[MolGraph], q: Queue):
    for mol in batch:
        for node1, node2 in mol.merging_graph.edges:
            atom_indices = mol.merging_graph.nodes[node1]["atom_indices"].union(
                mol.merging_graph.nodes[node2]["atom_indices"]
            )
...
def get_stats(
    ...
    if num_workers == 1:
        ...
        q = Queue()
        producers = [
            Process(target=get_stats_producer, args=(batches[i], q))
...
def update_stats(
    ...
    for n in neighbors2:
        if n != node1:
            atom_indices = graph.nodes[node2]["atom_indices"].union(
                graph.nodes[n]["atom_indices"]
            )
            motif_smiles = fragment2smiles(mol, atom_indices)
            stats[motif_smiles] -= 1
    ...
    for n in neighbors:
        atom_indices = new_graph.nodes[node1]["atom_indices"].union(
            new_graph.nodes[n]["atom_indices"]
        )
        motif_smiles = fragment2smiles(mol, atom_indices)
        stats[motif_smiles] += 1
        indices[motif_smiles][i] += 1
...
def apply_merging_operation_consumer(
    mols: List[MolGraph],
    stats: Dict[str, int],
    indices: Dict[str, Dict[int, int]],
    q: Queue,
...
def apply_merging_operation(
    ...
    if num_workers > 1:
        ...
        batches = [
            mols_to_process[i : i + batch_size]
            for i in range(0, len(mols_to_process), batch_size)
        ]
        q = Queue()
        producers = [
            Process(
                target=apply_merging_operation_producer, args=(motif, batches[i], q)
            )
            for i in range(num_workers)
        ]
        [p.start() for p in producers]
...
def merging_operation_learning(
    ...
    print(
        f"Number of workers: {num_workers}. Total number of CPUs: {mp.cpu_count()}.\n"
    )
    ...
    for i in range(num_iters):
        ...
        if stats[motif] < min_frequency:
            ...
            break
        print(
            f"[Iteration {i}] Most frequent motif: {motif}, frequency: {stats[motif]}.\n"
        )
